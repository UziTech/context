...
export default class Context {
	 ...
	 * @param {string} item.title (optional) title. Defaults to last item in keyPath.
	 * @param {string} [item.description=""] (optional) Description of command.
	 * @param {string} [item.selector="atom-workspace, atom-text-editor, .tree-view, .tab-bar"] (optional) Selector
	 * @param {boolean} [item.enabled=true] (optional) Enable menu item. Disabled menu items typically appear grayed out.
	 * @param {boolean} [item.visible=true] (optional) Should appear in the menu.
	 * @param {function} item.created (optional) Called on the item each time a context menu is created via a right click. You can assign properties to this to dynamically compute the command, label, etc.
	 * @param {function} item.shouldDisplay (optional) Called to determine whether to display this item on a given context menu deployment.
